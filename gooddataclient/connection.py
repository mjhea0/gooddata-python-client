...
class Connection(object):
    ...
    def request(self, uri, data=None, headers=JSON_HEADERS, method=None):
        ...
        if response.info().gettype() == 'application/json':
            responseX = json.loads(response.read())
            #print 'response (application/json):', responseX
            return responseX #json.loads(response.read())
        #print 'response (non application/json):', response
        return response
